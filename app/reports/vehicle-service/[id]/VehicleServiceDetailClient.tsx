...
import { ArrowLeft, Wrench, Calendar, Truck, FileText } from "lucide-react"
import jsPDF from "jspdf"
import html2canvas from "html2canvas"
import KopSurat from "@/app/components/KopSurat"

export default function VehicleServiceDetailClient({ data }: any) {
  ...
  const exportPDF = async () => {
    ...
    pdf.save(`service-report-${data.id}.pdf`)
  ...
  return (
    <>
      {/* HEADER */}
      <header className="border-b border-purple-500/20 pb-4 sm:pb-6 mb-6 sm:mb-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto flex flex-col sm:flex-row gap-3 sm:justify-between sm:items-center">
            <button
              onClick={() => router.back()}
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-purple-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl font-semibold shadow-lg"
            >
            ...
            <button
              onClick={exportPDF}
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-purple-700 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl font-semibold shadow-lg"
            >
              Export PDF
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-6 lg:px-8">
        <div
          ref={printRef}
          className="max-w-4xl mx-auto bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border border-purple-500/20 p-4 sm:p-6 md:p-8 text-black"
        >
          <KopSurat title="Laporan Servis Kendaraan Dinas" />

          {/* TITLE */}
          <div className="flex items-start sm:items-center gap-3 mb-6 sm:mb-8 pt-2 border-t border-purple-500/30">
            <Wrench className="w-7 h-7 sm:w-10 sm:h-10 text-purple-600" />
            <div>
              <h1 className="font-heading text-lg sm:text-2xl md:text-3xl font-bold text-purple-600">
                Detail Laporan #{data.id}
              </h1>
              <p className="text-xs sm:text-sm text-gray-600 mt-1">
                Data servis dan perawatan kendaraan
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8 text-sm">
            ...
            <div className="space-y-3">
              <div className="flex gap-3 p-3 sm:p-4 bg-purple-50 rounded-xl border border-purple-200">
