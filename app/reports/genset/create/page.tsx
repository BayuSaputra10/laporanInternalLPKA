...
export default function CreateGensetReport() {
  ...
  const inputClass =
    "w-full px-4 py-3 border border-gray-300 rounded-xl bg-white text-black placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 transition"
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "photo") {
      setForm(prev => ({ ...prev, photo: files ? files[0] : null }))
      return
    }
  ...
  const validate = (): Errors => {
    ...
    if (!form.waktuPemakaianJam || form.waktuPemakaianJam <= 0)
      newErrors.waktuPemakaianJam = "Harus lebih dari 0"

    if (form.hourMeterAwal == null || form.hourMeterAwal < 0)
      newErrors.hourMeterAwal = "Tidak valid"
    ...
    if (
      ...
      newErrors.hourMeterAkhir = "Harus lebih besar dari awal"
    ...
    if (
      ...
      newErrors.solarLevelAwal = "0 - 100%"
    ...
    if (
      ...
      newErrors.solarLevelAkhir = "0 - 100%"
    ...
    if (
      ...
      newErrors.solarLevelAkhir = "Tidak boleh lebih besar dari awal"
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await fetch("/api/reports/genset", {
        method: "POST",
        body: formData
      })
